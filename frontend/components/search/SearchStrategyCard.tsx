'use client';

import { motion } from 'framer-motion';
import { Search, MapPin, Hash } from 'lucide-react';

interface Strategy {
  roles: string[];
  locations: string[];
  keywords: string[];
  query?: string;
  source?: string;
}

interface Props {
  strategy: Strategy | null;
}

export default function SearchStrategyCard({ strategy }: Props) {
  if (!strategy) return null;

  return (
    <motion.div
      initial={{ opacity: 0, y: 12 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ delay: 0.15 }}
      className="card p-5"
    >
      <div className="flex items-center gap-2 mb-4">
        <Search size={11} style={{ color: 'var(--accent)' }} />
        <span className="text-label">Search Strategy</span>
        {strategy.source && (
          <span className="ml-auto badge badge-neutral">{strategy.source}</span>
        )}
      </div>

      <div className="space-y-4">
        {/* Roles */}
        <div>
          <div className="text-[10px] uppercase tracking-wider font-semibold mb-2"
               style={{ color: 'var(--text-faint)' }}>
            Roles
          </div>
          <div className="flex flex-wrap gap-1.5">
            {strategy.roles.map((role, i) => (
              <span key={i} className="badge badge-primary">{role}</span>
            ))}
          </div>
        </div>

        {/* Locations */}
        <div>
          <div className="flex items-center gap-1.5 mb-2">
            <MapPin size={10} style={{ color: 'var(--text-faint)' }} />
            <span className="text-[10px] uppercase tracking-wider font-semibold"
                  style={{ color: 'var(--text-faint)' }}>
              Locations
            </span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {strategy.locations.map((loc, i) => (
              <span key={i} className="badge badge-green">{loc}</span>
            ))}
          </div>
        </div>

        {/* Keywords */}
        <div>
          <div className="flex items-center gap-1.5 mb-2">
            <Hash size={10} style={{ color: 'var(--text-faint)' }} />
            <span className="text-[10px] uppercase tracking-wider font-semibold"
                  style={{ color: 'var(--text-faint)' }}>
              Keywords
            </span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {strategy.keywords.map((kw, i) => (
              <span key={i} className="badge badge-neutral">{kw}</span>
            ))}
          </div>
        </div>
      </div>
    </motion.div>
  );
}
